...
and Node. The current candidate is `v1.0.0-rc.5`; the wire protocol remains
...
```bash
bash scripts/release-check.sh 1.0.0-rc.5
```

The default updater resolves the latest non-prerelease `v*` Release. An
explicit version may select `v1.0.0-rc.5` or a later stable tag. This permits
