...
LITE_MODE_MARKER=/etc/relay-panel/lite-mode
LITE_FALLBACK_ROOT=/var/www/fallback
LITE_FALLBACK_INDEX=/var/www/fallback/index.html
LITE_FALLBACK_CONF=/etc/nginx/conf.d/relay-panel-lite-fallback.conf

resolve_lite_mode() {
  local requested="${1:-0}" root="${2:-}" marker
  case "$requested" in
    0|1) ;;
    *) fail "LITE_MODE must be 0 or 1" ;;
  esac
  marker="$root$LITE_MODE_MARKER"
  EFFECTIVE_LITE_MODE=0
  if [ -e "$marker" ]; then
    [ -f "$marker" ] || fail "Lite mode marker is not a regular file"
    [ "$(cat "$marker")" = lite ] || fail "Lite mode marker is invalid"
    EFFECTIVE_LITE_MODE=1
  elif [ "$requested" = 1 ]; then
    if [ -e "$root/opt/relay-node/relay-node" ] \
      || [ -e "$root/etc/systemd/system/relay-node.service" ]; then
      fail "refusing to convert an existing Standard node to Lite mode"
    fi
    EFFECTIVE_LITE_MODE=1
  fi
}
...
capture_transaction() {
  ...
  snapshot_file lite_marker "$LITE_MODE_MARKER"
  snapshot_file lite_fallback_index "$LITE_FALLBACK_INDEX"
  snapshot_file lite_fallback_conf "$LITE_FALLBACK_CONF"
  ...
  snapshot_directory_state lite_fallback_dir "$LITE_FALLBACK_ROOT"
...
rollback_transaction() {
  ...
  for item in \
    ...
    "lite_marker:$LITE_MODE_MARKER" \
    "lite_fallback_index:$LITE_FALLBACK_INDEX" \
    "lite_fallback_conf:$LITE_FALLBACK_CONF"
  do
    key="${item%%:*}"
    path="${item#*:}"
  ...
  restore_file_if_changed lite_marker "$LITE_MODE_MARKER" || failed=1
  restore_file_if_changed lite_fallback_index "$LITE_FALLBACK_INDEX" || failed=1
  restore_file_if_changed lite_fallback_conf "$LITE_FALLBACK_CONF" || failed=1
  ...
  file_matches_snapshot lite_marker "$LITE_MODE_MARKER" || failed=1
  file_matches_snapshot lite_fallback_index "$LITE_FALLBACK_INDEX" || failed=1
  file_matches_snapshot lite_fallback_conf "$LITE_FALLBACK_CONF" || failed=1
  restore_directory_state lite_fallback_dir "$LITE_FALLBACK_ROOT" || failed=1
  ...
  remove_file_staging "$LITE_MODE_MARKER"
...
is_managed_camouflage_config() {
  local path="$1"
  [ -f "$path" ] || return 1
  grep -qE '^# (generated by relay-node; TLS camouflage sites|RelayPanel managed bootstrap camouflage fallback)$' "$path"
}
...
is_managed_listener_config() {
  ...
  case "$port" in
    ...
    80)
      ...
      ;;
    5245)
      grep -qF '# RelayPanel managed Lite fallback' \
        "$(nginx_path "$LITE_FALLBACK_CONF")" 2>/dev/null
      ;;
    *) return 1 ;;
  esac
}
...
preflight_managed_ports() {
  ...
  if [ "${EFFECTIVE_LITE_MODE:-0}" = 1 ]; then
    ensure_listener_is_managed_or_free 5245
  fi
}

ensure_lite_fallback() {
  local root index conf index_tmp conf_tmp
  root="$(managed_path "$LITE_FALLBACK_ROOT")"
  index="$(managed_path "$LITE_FALLBACK_INDEX")"
  conf="$(managed_path "$LITE_FALLBACK_CONF")"
  index_tmp="$index.tmp"
  conf_tmp="$conf.tmp"
  if [ -e "$index" ]; then
    [ -f "$index" ] \
      && grep -qF '<!-- RelayPanel managed Lite fallback -->' "$index" \
      || fail "LITE_FALLBACK_CONFLICT: static index is not Reality Panel-managed"
  fi
  if [ -e "$conf" ]; then
    [ -f "$conf" ] \
      && grep -qF '# RelayPanel managed Lite fallback' "$conf" \
      || fail "LITE_FALLBACK_CONFLICT: Nginx config is not Reality Panel-managed"
  fi
  install -d -m 0755 "$root" "$(dirname "$conf")"
  cat > "$index_tmp" <<'EOF'
<!-- RelayPanel managed Lite fallback -->
<!doctype html>
<html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Welcome</title></head><body><h1>Welcome</h1></body></html>
EOF
...
server {
    ...
    location / { proxy_pass http://127.0.0.1:5245; }
}
EOF
  mv -f "$tmp" "$conf"
  NGINX_RELOAD_REQUIRED=1
...
if [ "${1:-}" = "--test-lite-fallback" ]; then
  TRANSACTION_ROOT="${2:?test root required}"
  ensure_lite_fallback
  exit 0
fi

if [ "${1:-}" = "--test-lite-mode" ]; then
  resolve_lite_mode "${3:-0}" "${2:?test root required}"
  printf '%s\n' "$EFFECTIVE_LITE_MODE"
  exit 0
fi
...
if [ "$EFFECTIVE_LITE_MODE" = 1 ]; then
  apt-get install -y -qq nginx libnginx-mod-stream openssl
else
  apt-get install -y -qq docker.io nginx libnginx-mod-stream openssl apparmor
fi
if ! command -v modprobe >/dev/null 2>&1; then
  apt-get install -y -qq kmod || warn "native BBR module loader is unavailable; continuing"
fi
step_ok
...
if [ "$EFFECTIVE_LITE_MODE" = 1 ]; then
  printf 'lite\n' > "$LITE_MODE_MARKER.tmp"
  chmod 0644 "$LITE_MODE_MARKER.tmp"
  mv -f "$LITE_MODE_MARKER.tmp" "$LITE_MODE_MARKER"
fi
...
if [ "$EFFECTIVE_LITE_MODE" = 1 ]; then
  ...
  relay_after='network-online.target docker.service nginx.service'
fi
...
if [ "$EFFECTIVE_LITE_MODE" = 0 ]; then
  ...
  /opt/relay-node/relay-node --prepare-host-runtime
  step_ok
fi
...
step capabilities
# `openlist` is the historical wire slot for the required local camouflage
# capability. Xiaoya or the Lite static fallback satisfies it.
