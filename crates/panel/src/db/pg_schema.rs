...
-- credential_id is a public locator, not a bearer secret. S2-A2A fixes the
-- internal V1 verifier format while keeping issuance/authentication runtime inert.
...
pub async fn run_pg_migrations(pool: &sqlx::PgPool) -> Result<(), sqlx::Error> {
    ...
    if current < 37 {
        let mut tx = pool.begin().await?;
        sqlx::query("ALTER TABLE node_credentials ADD COLUMN IF NOT EXISTS activated_at TEXT")
            .execute(&mut *tx)
            .await?;
        sqlx::query(
            "CREATE UNIQUE INDEX IF NOT EXISTS uq_node_credentials_one_active \
        ...
        .await?;
        sqlx::query(
            "INSERT INTO schema_version (version) VALUES (37) ON CONFLICT (version) DO NOTHING",
        )
        .execute(&mut *tx)
        .await?;
        tx.commit().await?;
...
mod tests {
    ...
    #[test]
    fn pg_schema_contains_node_credential_lifecycle_foundation() {
        ...
        assert!(PG_SCHEMA_SQL.contains("activated_at TEXT"));
        assert!(PG_SCHEMA_SQL.contains("revoked_at TEXT"));
        assert!(PG_SCHEMA_SQL.contains("uq_node_credentials_one_active"));
