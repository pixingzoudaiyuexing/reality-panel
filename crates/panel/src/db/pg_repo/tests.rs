...
async fn pg_dns_record_bindings_preserve_exact_ownership_and_enforce_uniqueness() {
    ...
    adoption.line_key = "wrong-line".into();
    assert_eq!(
        db.adopt_detached_dns_record_binding(&adoption)
            .await
            .unwrap(),
    ...
    adoption.line_key = "default".into();
    assert_eq!(
        db.adopt_detached_dns_record_binding(&adoption)
            .await
            .unwrap(),
...
#[tokio::test]
async fn pg_ambiguous_dns_record_rebind_requires_complete_terminal_identity() {
...
fn pg_test_node_credential_candidate(
    credential_id: &str,
    home_group_id: i64,
    node_id: &str,
    secret_byte: u8,
...
async fn pg_node_credential_schema_constraints_and_fk_restrict() {
    ...
    db.allocate_node_credential_candidate(&pg_test_node_credential_candidate(
        "cred-a",
        10,
        "my-fixed-id-12345",
        0x41,
    ...
    assert!(sqlx::query(
        "INSERT INTO node_credentials          (credential_id, home_group_id, node_id, generation, verifier_format, verifier_version, verifier_data)          VALUES ('invalid-long', 10, $1, 99, 'rp-node-sha256', 1, $2)",
    )
    .bind(&overlong)
    .bind(vec![1_u8; 32])
    .execute(&db.pool)
    ...
    assert!(sqlx::query(
        "INSERT INTO node_credentials          (credential_id, home_group_id, node_id, generation, verifier_format, verifier_version, verifier_data)          VALUES ('cred-duplicate-generation', 10, 'my-fixed-id-12345', 1, 'rp-node-sha256', 1, $1)",
    )
    .bind(vec![2_u8; 32])
    .execute(&db.pool)
    .await
    ...
    for (credential_id, generation, version, verifier_data) in [
        ...
        assert!(sqlx::query(
            "INSERT INTO node_credentials \
             (credential_id, home_group_id, node_id, generation, verifier_format, verifier_version, verifier_data) \
             VALUES ($1, 10, 'BBB', $2, 'rp-node-sha256', $3, $4)",
        )
        .bind(credential_id)
        .bind(generation)
        .bind(version)
...
#[tokio::test]
async fn pg_node_credential_lifecycle_contract() {
    let Some(db) = repo("node_credential_lifecycle").await else {
        return;
    };
    seed_group(&db, 10).await;
    seed_group(&db, 20).await;
    ...
    let direct_second_active_before_replace = sqlx::query(
        "UPDATE node_credentials SET activated_at = to_char(now() AT TIME ZONE 'UTC', 'YYYY-MM-DD HH24:MI:SS')          WHERE credential_id = 'cred-life-2'",
    )
    .execute(&db.pool)
    .await;
    assert!(
...
#[tokio::test]
async fn pg_node_credential_replacement_rejects_generation_rollback() {
    let Some(db) = repo("node_credential_generation_rollback").await else {
        return;
    };
    seed_group(&db, 10).await;
...
#[tokio::test]
async fn pg_node_credential_activation_history_blocks_ordinary_recovery_and_is_group_scoped() {
    let Some(db) = repo("node_credential_activation_history").await else {
        return;
    };
    seed_group(&db, 10).await;
    seed_group(&db, 20).await;
...
#[tokio::test]
async fn pg_node_credential_replacement_rejects_below_historical_high() {
    let Some(db) = repo("node_credential_historical_high").await else {
        return;
    };
    seed_group(&db, 10).await;
    ...
    sqlx::query(
        "UPDATE node_credentials          SET activated_at = to_char(now() AT TIME ZONE 'UTC', 'YYYY-MM-DD HH24:MI:SS'),              revoked_at = to_char(now() AT TIME ZONE 'UTC', 'YYYY-MM-DD HH24:MI:SS')          WHERE credential_id = 'cred-legacy-history'",
    )
    .execute(&db.pool)
    .await
    .unwrap();
    sqlx::query(
        "UPDATE node_credentials          SET activated_at = to_char(now() AT TIME ZONE 'UTC', 'YYYY-MM-DD HH24:MI:SS')          WHERE credential_id = 'cred-legacy-current'",
    )
    .execute(&db.pool)
    .await
    .unwrap();
...
#[tokio::test]
async fn pg_node_credential_concurrent_replacement_keeps_single_active() {
    let Some(db) = repo("node_credential_replacement_race").await else {
        return;
    };
    seed_group(&db, 10).await;
...
#[tokio::test]
async fn pg_node_credential_concurrent_generation_allocation() {
    let Some(db) = repo("node_credential_concurrent_generation").await else {
        return;
    };
    seed_group(&db, 10).await;
    let a = pg_test_node_credential_candidate("cred-race-a", 10, "AAA", 0x61);
    let b = pg_test_node_credential_candidate("cred-race-b", 10, "AAA", 0x62);
...
async fn pg_node_credential_schema_upgrade_is_idempotent() {
    ...
    let activated_columns: i64 = sqlx::query_scalar(
        "SELECT COUNT(*) FROM information_schema.columns \
         WHERE table_schema = 'public' AND table_name = 'node_credentials' AND column_name = 'activated_at'",
    )
    .fetch_one(&pool)
    .await
    .unwrap();
    let active_indexes: i64 = sqlx::query_scalar(
        "SELECT COUNT(*) FROM pg_indexes \
         WHERE schemaname = 'public' AND tablename = 'node_credentials' \
           AND indexname = 'uq_node_credentials_one_active'",
    )
    .fetch_one(&pool)
    .await
    .unwrap();
...
#[tokio::test]
async fn pg_node_credential_lifecycle_upgrade_preserves_inactive_rows() {
    let Some(pool) = fresh_pool("node_credential_lifecycle_upgrade").await else {
        return;
    };
    sqlx::query("CREATE TABLE schema_version (version INTEGER PRIMARY KEY)")
        .execute(&pool)
    ...
    sqlx::query("CREATE TABLE device_groups (id BIGINT PRIMARY KEY)")
        ...
        .await
        .unwrap();
    sqlx::query("INSERT INTO device_groups (id) VALUES (30)")
        .execute(&pool)
        .await
        .unwrap();
    sqlx::query(
        "CREATE TABLE node_credentials (             credential_id TEXT PRIMARY KEY,             home_group_id BIGINT NOT NULL REFERENCES device_groups(id) ON DELETE RESTRICT,             node_id TEXT NOT NULL,             generation BIGINT NOT NULL,             verifier_format TEXT NOT NULL,             verifier_version BIGINT NOT NULL,             verifier_data BYTEA NOT NULL,             created_at TEXT NOT NULL DEFAULT '',             updated_at TEXT NOT NULL DEFAULT '',             revoked_at TEXT,             UNIQUE (home_group_id, node_id, generation)         )",
    )
    .execute(&pool)
    .await
    .unwrap();
    sqlx::query(
        "INSERT INTO node_credentials \
         (credential_id, home_group_id, node_id, generation, verifier_format, verifier_version, verifier_data) \
         VALUES ('legacy-candidate', 30, 'AAA', 1, 'test-only-opaque', 1, $1)",
    )
    .bind(vec![0xA5_u8; 32])
    .execute(&pool)
    .await
...
fn pg_test_credential_delivery_requests(
    claim_id: &str,
    home_group_id: i64,
    node_id: &str,
...
#[tokio::test]
async fn pg_node_credential_delivery_prepare_activate_and_response_loss_contract() {
    let Some(db) = repo("node_credential_delivery_core").await else {
        return;
    };
    ...
    let t0 = pg_claim_test_time("2026-09-23T18:00:00Z");
    let (claim, claim_secret) =
        pg_test_node_claim("b2a-core-claim", 60, "B2A_CORE", 0x21, 1, t0, 600);
    assert!(matches!(
    ...
    assert_eq!(
        db.find_node_credential_claim("b2a-core-claim")
            .await
            .unwrap()
            .unwrap()
            .state,
...
#[tokio::test]
async fn pg_node_credential_delivery_history_cancel_and_independent_expiry_contract() {
    let Some(db) = repo("node_credential_delivery_boundaries").await else {
        return;
    };
    for group in [61_i64, 62, 63, 64] {
        seed_group(&db, group).await;
    }
    let t0 = pg_claim_test_time("2026-09-23T19:00:00Z");
    ...
    db.activate_node_credential(&active.credential_id, 61, &active_node, active.generation)
        .await
        .unwrap();
    let (claim, secret) = pg_test_node_claim("claim-active", 61, "B2A_ACTIVE", 0x62, 1, t0, 600);
    db.create_node_credential_claim(&claim).await.unwrap();
    db.claim_node_credential(&pg_test_claim_attempt(
        "claim-active",
        61,
    ...
    .await
    .unwrap();
    let (claim, secret) = pg_test_node_claim("claim-history", 62, "B2A_HISTORY", 0x67, 1, t0, 600);
    db.create_node_credential_claim(&claim).await.unwrap();
    db.claim_node_credential(&pg_test_claim_attempt(
        "claim-history",
        62,
    ...
    let (claim, secret) = pg_test_node_claim("claim-cancel", 63, "B2A_CANCEL", 0x70, 1, t0, 600);
    db.create_node_credential_claim(&claim).await.unwrap();
    db.claim_node_credential(&pg_test_claim_attempt(
        "claim-cancel",
        63,
    ...
    let (claim, secret) = pg_test_node_claim("claim-expiry", 64, "B2A_EXPIRY", 0x80, 1, t0, 600);
    db.create_node_credential_claim(&claim).await.unwrap();
    db.claim_node_credential(&pg_test_claim_attempt(
        "claim-expiry",
        64,
...
async fn pg_node_credential_delivery_concurrent_prepare_and_activate_converges() {
    ...
    let t0 = pg_claim_test_time("2026-09-23T20:00:00Z");
    let (claim, secret) = pg_test_node_claim("claim-race", 65, "B2A_RACE", 0x90, 1, t0, 600);
    db.create_node_credential_claim(&claim).await.unwrap();
    db.claim_node_credential(&pg_test_claim_attempt(
        "claim-race",
        65,
...
#[tokio::test]
async fn pg_node_credential_delivery_db_error_rolls_back_direct_active_insert() {
    let Some(db) = repo("node_credential_delivery_db_error").await else {
        return;
    };
    seed_group(&db, 66).await;
    let t0 = pg_claim_test_time("2026-09-23T21:00:00Z");
    let (claim, secret) = pg_test_node_claim("claim-db-error", 66, "B2A_DBERR", 0xa0, 1, t0, 600);
    db.create_node_credential_claim(&claim).await.unwrap();
    db.claim_node_credential(&pg_test_claim_attempt(
        "claim-db-error",
        66,
    ...
    sqlx::query(
        "ALTER TABLE node_credential_deliveries \
         ADD CONSTRAINT fail_b2a_completion CHECK (state <> 'COMPLETED')",
    )
    .execute(&db.pool)
    .await
    .unwrap();
    ...
    assert!(
        db.activate_initial_node_credential_from_delivery(&activate)
            .await
            .is_err(),
        "forced PG constraint error after ACTIVE insert must roll back the transaction"
    );
    assert!(db
        .find_node_credential("db-error-credential")
        .await
        .unwrap()
        .is_none());
    assert_eq!(
    ...
    sqlx::query("ALTER TABLE node_credential_deliveries DROP CONSTRAINT fail_b2a_completion")
        .execute(&db.pool)
        .await
        .unwrap();
    assert!(matches!(
...
#[tokio::test]
async fn pg_node_credential_delivery_future_cancellation_rolls_back_and_releases_locks() {
    let Some(db) = repo("node_credential_delivery_future_cancel").await else {
        return;
    };
    seed_group(&db, 67).await;
    let t0 = pg_claim_test_time("2026-09-23T22:00:00Z");
    let (claim, secret) = pg_test_node_claim(
        "claim-cancel-future",
        67,
        "B2A_CANCEL_FUT",
    ...
    let task_db = PgRepository::new(db.pool.clone());
    ...
    tokio::time::timeout(std::time::Duration::from_secs(5), pause.reached.notified())
        .await
        .expect("PG activation must reach deterministic post-insert pause");
    task.abort();
    ...
    assert!(
        db.find_node_credential("cancel-future-credential")
            ...
            .unwrap()
            .is_none(),
        "cancelling the PG Future after credential INSERT must roll back the INSERT"
    );
    assert_eq!(
...
#[tokio::test]
async fn pg_node_credential_delivery_migration_preserves_previous_claim_history() {
    let Some(pool) = fresh_pool("node_delivery_v38_upgrade").await else {
        return;
    };
    sqlx::query("CREATE TABLE schema_version (version INTEGER PRIMARY KEY)")
        .execute(&pool)
        .await
        .unwrap();
    sqlx::query("INSERT INTO schema_version (version) VALUES (38)")
        .execute(&pool)
        .await
        .unwrap();
    sqlx::query("CREATE TABLE device_groups (id BIGINT PRIMARY KEY)")
        ...
        .await
        .unwrap();
    sqlx::query("INSERT INTO device_groups (id) VALUES (71)")
        .execute(&pool)
        .await
        .unwrap();
    sqlx::query("CREATE TABLE node_credentials (credential_id TEXT PRIMARY KEY)")
        ...
        .await
        .unwrap();
    sqlx::query("INSERT INTO node_credentials (credential_id) VALUES ('v38-history-credential')")
        .execute(&pool)
        .await
        .unwrap();
    sqlx::query(
        "CREATE TABLE node_credential_claims (\
             ...
             approved_by BIGINT NOT NULL, approval_ref TEXT NOT NULL CHECK (char_length(approval_ref) BETWEEN 1 AND 128),\
    ...
    .await
    .unwrap();
    let secret_verifier = vec![0xd1_u8; 32];
    let nonce_verifier = vec![0xd2_u8; 32];
    sqlx::query(
        "INSERT INTO node_credential_claims (\
         ...
         ) VALUES ('v38-claimed',71,'V38_NODE','rp-node-claim-sha256',1,$1,'CLAIMED',\
                   '2026-09-23T12:00:00Z','rp-node-claim-nonce-sha256',1,$2,88,'approval-v38',\
    ...
    assert_eq!(claim.approved_by, 88);
    assert_eq!(claim.approval_ref, "approval-v38");
    ...
    let delivery_table: i64 = sqlx::query_scalar(
        "SELECT COUNT(*) FROM information_schema.tables WHERE table_schema='public' AND table_name='node_credential_deliveries'",
    )
    .fetch_one(&db.pool)
    .await
    .unwrap();
    let delivery_index: i64 = sqlx::query_scalar(
        "SELECT COUNT(*) FROM pg_indexes WHERE schemaname='public' AND indexname='uq_node_credential_deliveries_one_prepared'",
    )
    .fetch_one(&db.pool)
    .await
    .unwrap();
    let credential_history: i64 = sqlx::query_scalar(
        "SELECT COUNT(*) FROM node_credentials WHERE credential_id='v38-history-credential'",
    )
    .fetch_one(&db.pool)
    .await
    .unwrap();
    ...
    assert_eq!(credential_history, 1);
    cleanup(&db).await;
}
#[tokio::test]
