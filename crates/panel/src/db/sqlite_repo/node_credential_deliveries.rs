...
async fn lock_group_writer(
    ...
    // Match Claim serialization: the first statement after BEGIN is a write so
    // no read snapshot can be established before SQLite's single-writer lock.
...
async fn fetch_delivery_tx(
    ...
    let sql =
        format!("SELECT {DELIVERY_COLUMNS} FROM node_credential_deliveries WHERE claim_id = ?");
...
async fn fetch_credential_tx(
    ...
    let sql = format!("SELECT {CREDENTIAL_COLUMNS} FROM node_credentials WHERE credential_id = ?");
...
#[async_trait]
impl NodeCredentialDeliveryRepository for SqliteRepository {
    ...
    async fn prepare_initial_node_credential_delivery(
        ...
        let activation_history: i64 = sqlx::query_scalar(
            "SELECT COUNT(*) FROM node_credentials \
             WHERE home_group_id=? AND node_id=? AND activated_at IS NOT NULL",
        )
        .bind(request.home_group_id)
        .bind(request.node_id.as_str())
        .fetch_one(&mut *tx)
        ...
        let credential_id_exists: i64 =
            sqlx::query_scalar("SELECT COUNT(*) FROM node_credentials WHERE credential_id=?")
        ...
        let claim_update = sqlx::query(
            "UPDATE node_credential_claims \
             SET state='CREDENTIAL_PENDING', credential_pending_at=?, updated_at=? \
             WHERE claim_id=? AND home_group_id=? AND node_id=? \
    ...
    async fn activate_initial_node_credential_from_delivery(
        ...
        let sql = format!(
            "INSERT INTO node_credentials (\
             ...
             ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?) RETURNING {CREDENTIAL_COLUMNS}"
        );
        ...
        let sql = format!(
            "UPDATE node_credential_deliveries \
             SET state='COMPLETED', credential_generation=?, proof_verified_at=?, \
                 completed_at=?, updated_at=? \
             WHERE claim_id=? AND home_group_id=? AND node_id=? AND credential_id=? \
               AND state='PREPARED' AND expires_at > ? \
             RETURNING {DELIVERY_COLUMNS}"
        );
