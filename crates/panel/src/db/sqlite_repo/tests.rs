...
#[tokio::test]
async fn traffic_batch_idempotency_contract() {
    let path = std::env::temp_dir().join(format!(
        "reality-panel-t1-traffic-{}.db",
        uuid::Uuid::new_v4()
    ));
    ...
    for (credential_id, home_group_id) in [("cred-a", 10_i64), ("cred-b", 20_i64)] {
        sqlx::query(
            "INSERT INTO node_credentials
             ...
             VALUES (?, ?, 'NODE_T1', 1, 'rp-node-sha256', 1, ?, datetime('now'))",
        )
        .bind(credential_id)
        .bind(home_group_id)
    ...
    assert_eq!(
        db.apply_idempotent_traffic_batch(&valid_retry_scope, &valid_retry)
            ...
            .unwrap(),
        IdempotentTrafficBatchResult::Applied,
        "a rolled-back first attempt must not permanently occupy batch_id"
    );
    ...
    assert_eq!(
        db.apply_idempotent_traffic_batch(&partial_scope, &partial_invalid)
            ...
            .unwrap(),
        IdempotentTrafficBatchResult::Unavailable,
        "one foreign entry must roll back the whole strict batch"
    );
    ...
    assert_eq!(
        db.apply_idempotent_traffic_batch(&missing_scope, &missing_entries)
            ...
            .unwrap(),
        IdempotentTrafficBatchResult::Unavailable,
        "missing and foreign rules share the same rejection result"
    );
    ...
    // Same textual node_id and batch_id are independent under another Home
    // Group + authenticated Credential scope.
    ...
    for batch_id in [
        ...
        let count: i64 =
            sqlx::query_scalar("SELECT COUNT(*) FROM traffic_report_batches WHERE batch_id = ?")
                .bind(batch_id)
                .fetch_one(&pool)
                .await
                .unwrap();
        assert_eq!(count, 0, "rejected batch must leave no dedupe success row");
    }

    // Database reconnect proves a committed dedupe record survives process/DB
    // pool restart and still prevents a second accounting application.
    pool.close().await;
    let reopened_pool = SqlitePoolOptions::new()
        ...
        .await
        .unwrap();
    let reopened = SqliteRepository::new(reopened_pool.clone());
    assert_eq!(
        reopened
...
async fn traffic_history_preserves_same_hour_rows_across_group_move() {
    let db = repo().await;
    let uid = seed_history_fixture(&db, "hist_move", 73, 305, 1.0).await;
    sqlx::query(
        "INSERT INTO device_groups (id, name, group_type, token, uid, rate) \
         VALUES (74, 'hist-move-new', 'in', 'tok-74', ?, 1.0)",
    )
    .bind(uid)
    .execute(&db.pool)
    .await
...
#[tokio::test]
async fn dns_record_bindings_preserve_exact_ownership_and_enforce_uniqueness() {
    let db = repo().await;
    sqlx::query("PRAGMA foreign_keys = ON")
        .execute(&db.pool)
        .await
    ...
    for (id, port) in [(100, 21000), (101, 21001)] {
        sqlx::query(
            "INSERT INTO forward_rules \
             (id, name, uid, listen_port, device_group_in, target_addr, target_port) \
    ...
    let original = new_dns_binding(100, "record-1");
    let id = db.insert_dns_record_binding(&original).await.unwrap();
    let by_record = db
    ...
    assert_eq!(by_record.line, "Default");
    assert_eq!(by_record.line_key, "default");
    assert!(db
    ...
    let same_provider_record = new_dns_binding(101, "record-1");
    assert!(matches!(
        db.insert_dns_record_binding(&same_provider_record).await,
        Err(DbError::UniqueViolation)
    ));
    let same_logical_record = new_dns_binding(100, "record-2");
    assert!(matches!(
        db.insert_dns_record_binding(&same_logical_record).await,
        Err(DbError::UniqueViolation)
    ));
    ...
    let updated = db
        ...
        .unwrap()
        .unwrap();
    assert_eq!(updated.state, "MISSING");
    assert_eq!(updated.last_error_category.as_deref(), Some("NOT_FOUND"));
    ...
    db.insert_dns_record_sync(&NewDnsRecordSync {
        ...
        created_at: "2026-08-26 00:02:00".into(),
        updated_at: "2026-08-26 00:02:00".into(),
    })
    .await
    .unwrap();
...
#[tokio::test]
async fn ambiguous_dns_record_rebind_requires_complete_terminal_identity() {
    let db = repo().await;
    sqlx::query("PRAGMA foreign_keys = ON")
        .execute(&db.pool)
        .await
    ...
    let id = db
        .insert_dns_record_binding(&new_dns_binding(100, "stale-old"))
        .await
        .unwrap();
    db.update_dns_record_binding_observation(
        id,
    ...
    let exact = AmbiguousDnsRecordBindingRebind {
        binding_id: id,
        rule_id: 100,
        fqdn: "op1.example.com".into(),
        zone_id: 7,
        ...
        line: "Default".into(),
        line_key: "default".into(),
        previous_record_id: "stale-old".into(),
    ...
    wrong.binding_id += 1000;
    mismatches.push(wrong);
    let mut wrong = exact.clone();
    wrong.rule_id = 101;
    mismatches.push(wrong);
    let mut wrong = exact.clone();
    wrong.fqdn = "other.example.com".into();
    mismatches.push(wrong);
    let mut wrong = exact.clone();
    wrong.zone_id = 8;
    mismatches.push(wrong);
    let mut wrong = exact.clone();
    wrong.zone_name = "other.example".into();
    mismatches.push(wrong);
    let mut wrong = exact.clone();
    wrong.host = "other".into();
    mismatches.push(wrong);
    let mut wrong = exact.clone();
    wrong.record_type = "AAAA".into();
    mismatches.push(wrong);
    let mut wrong = exact.clone();
    wrong.line = "Other".into();
    mismatches.push(wrong);
    let mut wrong = exact.clone();
    wrong.line_key = "dnsmgr:Other".into();
    mismatches.push(wrong);
    let mut wrong = exact.clone();
    wrong.previous_record_id = "wrong-old".into();
    mismatches.push(wrong);
    let mut wrong = exact.clone();
    wrong.desired_value = "192.0.2.99".into();
    mismatches.push(wrong);
    let mut wrong = exact.clone();
    wrong.replacement_record_id = "stale-old".into();
    mismatches.push(wrong);

    for mismatch in mismatches {
        assert_eq!(
            db.rebind_ambiguous_dns_record_binding(&mismatch)
                .await
                .unwrap(),
            0,
            "any stale identity mismatch must affect zero rows"
        );
    }
    ...
    assert_eq!(
        db.rebind_ambiguous_dns_record_binding(&exact)
            ...
            .unwrap(),
        0,
        "ERROR without MUTATION_UNKNOWN is not recoverable"
    );
    db.update_dns_record_binding_observation(
        id,
        "ERROR",
    ...
    assert_eq!(recovered.updated_at, "2026-09-25 00:01:00");
    assert_eq!(
        db.rebind_ambiguous_dns_record_binding(&exact)
            .await
            .unwrap(),
    ...
    sqlx::query(
        "UPDATE dns_record_bindings SET record_id = 'stale-again', state = 'ERROR', \
         last_error_category = 'MUTATION_UNKNOWN' WHERE id = ?",
    )
    .bind(id)
    .execute(&db.pool)
    .await
    .unwrap();
    db.insert_dns_record_binding(&new_dns_binding(101, "claimed"))
        .await
        .unwrap();
    let mut conflict = exact;
    conflict.previous_record_id = "stale-again".into();
    conflict.replacement_record_id = "claimed".into();
    assert!(matches!(
        db.rebind_ambiguous_dns_record_binding(&conflict).await,
        Err(DbError::UniqueViolation)
    ));
}

#[tokio::test]
async fn dns_record_sync_state_is_durable_and_due_queries_are_bounded() {
    let db = repo().await;
    seed_group(&db, 10).await;
    sqlx::query(
        "INSERT INTO forward_rules \
...
fn test_node_credential_candidate(
    credential_id: &str,
    home_group_id: i64,
    node_id: &str,
    secret_byte: u8,
) -> NewNodeCredentialCandidate {
    let node_id = crate::node_identity::ReuseEligibleNodeId::parse(node_id).unwrap();
    let secret = crate::node_credential::NodeCredentialSecret::from_test_bytes([secret_byte; 32]);
...
#[tokio::test]
async fn node_credential_repository_contract() {
    let db = repo().await;
    sqlx::query("PRAGMA foreign_keys = ON")
        .execute(&db.pool)
        .await
    ...
    assert_eq!(other_group.generation, 1);
    assert_eq!(second.generation, 2);
    assert_eq!(first.verifier_format, "rp-node-sha256");
    assert_eq!(first.verifier_version, 1);
    assert_eq!(first.verifier_data.len(), 32);
    assert_ne!(first.verifier_data, vec![0x11; 32]);
    assert!(first.activated_at.is_none());
    ...
    let columns: Vec<String> =
        sqlx::query_scalar("SELECT name FROM pragma_table_info('node_credentials')")
            .fetch_all(&db.pool)
            .await
            .unwrap();
    assert!(columns.iter().any(|name| name == "activated_at"));
    assert!(!columns
        .iter()
...
#[tokio::test]
async fn node_credential_schema_constraints_and_fk_restrict() {
    let db = repo().await;
    sqlx::query("PRAGMA foreign_keys = ON")
        .execute(&db.pool)
        .await
    ...
    assert!(db
        ...
        .await
        .is_err());
    let existing = db.find_node_credential("cred-a").await.unwrap().unwrap();
    assert_eq!(existing.home_group_id, 10);
    assert_eq!(existing.node_id, "my-fixed-id-12345");
    ...
    assert!(sqlx::query(
        "INSERT INTO node_credentials          (credential_id, home_group_id, node_id, generation, verifier_format, verifier_version, verifier_data)          VALUES ('invalid-long', 10, ?, 99, 'rp-node-sha256', 1, ?)",
    )
    .bind(&overlong)
    .bind(vec![1_u8; 32])
    .execute(&db.pool)
    ...
    assert!(sqlx::query(
        "INSERT INTO node_credentials          (credential_id, home_group_id, node_id, generation, verifier_format, verifier_version, verifier_data)          VALUES ('cred-duplicate-generation', 10, 'my-fixed-id-12345', 1, 'rp-node-sha256', 1, ?)",
    )
    .bind(vec![2_u8; 32])
    .execute(&db.pool)
    .await
    ...
    for (credential_id, generation, version, verifier_data) in [
        ("bad-generation", 0_i64, 1_i64, vec![1_u8; 32]),
        ("bad-version", 20_i64, 0_i64, vec![1_u8; 32]),
        ("bad-verifier", 21_i64, 1_i64, Vec::new()),
    ] {
        assert!(sqlx::query(
            "INSERT INTO node_credentials \
             (credential_id, home_group_id, node_id, generation, verifier_format, verifier_version, verifier_data) \
             VALUES (?, 10, 'BBB', ?, 'rp-node-sha256', ?, ?)",
        )
        .bind(credential_id)
        .bind(generation)
        .bind(version)
...
#[tokio::test]
async fn node_credential_lifecycle_contract() {
    let db = repo().await;
    seed_group(&db, 10).await;
    seed_group(&db, 20).await;
    let node_id = crate::node_identity::ReuseEligibleNodeId::parse("AAA").unwrap();
    ...
    assert!(db
        ...
        .is_err());
    let still_first = db
        .find_node_credential("cred-life-1")
        .await
        .unwrap()
        .unwrap();
    ...
    assert_eq!(
        second.generation, 2,
        "failed insert must not consume a generation"
    );
    ...
    assert_eq!(
        db.activate_node_credential("cred-life-2", 10, &node_id, 2)
            .await
            .unwrap(),
        NodeCredentialMutationResult::Rejected
    );

    let direct_second_active_before_replace = sqlx::query(
        "UPDATE node_credentials SET activated_at = datetime('now')          WHERE credential_id = 'cred-life-2'",
    )
    .execute(&db.pool)
    .await;
    assert!(
        direct_second_active_before_replace.is_err(),
        "partial unique index must reject two active rows"
    );

    assert_eq!(
        db.replace_active_node_credential(10, &node_id, "cred-life-1", 99, "cred-life-2", 2)
            .await
            .unwrap(),
        NodeCredentialMutationResult::Rejected
    );
    let old = db
        ...
        .unwrap();
    let candidate = db
        .find_node_credential("cred-life-2")
        .await
        .unwrap()
        .unwrap();
    ...
    assert_eq!(
        db.replace_active_node_credential(10, &node_id, "cred-life-1", 1, "cred-life-2", 2)
            .await
            .unwrap(),
        NodeCredentialMutationResult::Applied
    );
    ...
    assert_eq!(
        third.generation, 3,
        "revocation must not permit generation reuse"
    );
    assert_eq!(
    ...
    assert_eq!(
        db.activate_node_credential("cred-life-other", 20, &node_id, 1)
            .await
            .unwrap(),
        NodeCredentialMutationResult::Applied
    );
...
#[tokio::test]
async fn node_credential_replacement_rejects_generation_rollback() {
    let db = repo().await;
    seed_group(&db, 10).await;
    let node_id = crate::node_identity::ReuseEligibleNodeId::parse("BBB").unwrap();
    ...
    let current = db
        ...
        .await
        .unwrap();
    assert_eq!((stale.generation, current.generation), (1, 2));
    assert_eq!(
        db.activate_node_credential("cred-rollback-current", 10, &node_id, 2)
            .await
            .unwrap(),
        NodeCredentialMutationResult::Applied
    );
    ...
    let stale = db
        .find_node_credential("cred-rollback-stale")
        .await
        .unwrap()
        .unwrap();
    let current = db
        .find_node_credential("cred-rollback-current")
        .await
        .unwrap()
        .unwrap();
...
#[tokio::test]
async fn node_credential_concurrent_generation_allocation() {
    let path = std::env::temp_dir().join(format!(
        "reality-panel-credential-concurrency-{}.db",
        uuid::Uuid::new_v4()
    ));
    let url = format!("sqlite://{}", path.display());
    let pool = crate::db::init::init_db(&url).await.unwrap();
    ...
    let a = test_node_credential_candidate("cred-race-a", 10, "AAA", 0x61);
    let b = test_node_credential_candidate("cred-race-b", 10, "AAA", 0x62);
    let (ra, rb) = tokio::join!(
        db.allocate_node_credential_candidate(&a),
        db.allocate_node_credential_candidate(&b)
    );
    let mut generations = vec![ra.unwrap().generation, rb.unwrap().generation];
...
#[tokio::test]
async fn node_credential_schema_upgrade_is_idempotent() {
    let db = repo().await;
    sqlx::query("DROP TABLE node_credentials")
        .execute(&db.pool)
        .await
        .unwrap();
    ...
    let activated_columns: i64 = sqlx::query_scalar(
        "SELECT COUNT(*) FROM pragma_table_info('node_credentials') WHERE name='activated_at'",
    )
    .fetch_one(&db.pool)
    .await
    .unwrap();
    let active_indexes: i64 = sqlx::query_scalar(
        "SELECT COUNT(*) FROM sqlite_master WHERE type='index' AND name='uq_node_credentials_one_active'",
    )
    .fetch_one(&db.pool)
    .await
    .unwrap();
    assert_eq!(activated_columns, 1);
    assert_eq!(active_indexes, 1);
}

#[tokio::test]
async fn node_credential_lifecycle_upgrade_preserves_inactive_rows() {
    let db = repo().await;
    sqlx::query("DROP TABLE node_credentials")
        .execute(&db.pool)
        .await
        .unwrap();
    sqlx::query(
        "CREATE TABLE node_credentials (             credential_id TEXT PRIMARY KEY CHECK (length(credential_id) BETWEEN 1 AND 128),             home_group_id INTEGER NOT NULL REFERENCES device_groups(id) ON DELETE RESTRICT,             node_id TEXT NOT NULL,             generation INTEGER NOT NULL CHECK (generation >= 1),             verifier_format TEXT NOT NULL,             verifier_version INTEGER NOT NULL CHECK (verifier_version >= 1),             verifier_data BLOB NOT NULL CHECK (length(verifier_data) > 0),             created_at TEXT NOT NULL DEFAULT (datetime('now')),             updated_at TEXT NOT NULL DEFAULT (datetime('now')),             revoked_at TEXT,             UNIQUE (home_group_id, node_id, generation)         )",
    )
    .execute(&db.pool)
    .await
    .unwrap();
    seed_group(&db, 30).await;
    sqlx::query(
        "INSERT INTO node_credentials \
         (credential_id, home_group_id, node_id, generation, verifier_format, verifier_version, verifier_data) \
         VALUES ('legacy-candidate', 30, 'AAA', 1, 'test-only-opaque', 1, ?)",
    )
    .bind(vec![0xA5_u8; 32])
    .execute(&db.pool)
    .await
    ...
    .await
    .unwrap();
    let legacy_activated: Option<String> = sqlx::query_scalar(
        "SELECT activated_at FROM node_credentials WHERE credential_id='legacy-candidate'",
    )
    .fetch_one(&db.pool)
    .await
    .unwrap();
    ...
    assert_eq!(active_indexes, 1);
    assert!(
        legacy_activated.is_none(),
        "existing S2-A1 rows must migrate as inactive"
    );
}

fn claim_test_time(value: &str) -> chrono::DateTime<chrono::Utc> {
...
#[tokio::test]
async fn node_credential_delivery_expiry_rejects_partial_commit_and_terminal_noops() {
    let db = repo().await;
    sqlx::query("PRAGMA foreign_keys = ON")
        .execute(&db.pool)
        .await
    ...
    let t0 = claim_test_time("2026-09-23T23:00:00Z");
    ...
    let fixture = sqlx::query(
        "UPDATE node_credential_claims SET state='CANCELLED', cancelled_at=?, updated_at=? \
         WHERE claim_id=? AND state='CREDENTIAL_PENDING'",
    )
    ...
    assert_eq!(
        sqlx::query_scalar::<_, i64>("SELECT COUNT(*) FROM node_credentials WHERE credential_id=?")
...
async fn node_credential_claim_repository_contract() {
    ...
    seed_group(&db, 10).await;
    seed_group(&db, 20).await;
    let now = claim_test_time("2026-09-23T12:00:00Z");

    let (claim, secret) = test_node_claim("claim-a", 10, "Node_A", 0x11, 1, now, 600);
    let created = db.create_node_credential_claim(&claim).await.unwrap();
    let NodeCredentialClaimCreateResult::Created(created) = created else {
        panic!("first claim must be created");
    };
    assert_eq!(created.state, "APPROVED");
    assert_ne!(created.secret_verifier_data, vec![0x11; 32]);
    assert!(created.claimant_nonce_verifier_data.is_none());
    ...
    let duplicate = db.create_node_credential_claim(&claim).await.unwrap();
    assert!(matches!(
        duplicate,
        NodeCredentialClaimCreateResult::Existing(_)
    ));

    let (other_group_claim, _) =
        test_node_claim("claim-other-group", 20, "Node_A", 0x12, 1, now, 600);
    assert!(matches!(
        db.create_node_credential_claim(&other_group_claim)
            .await
            .unwrap(),
        NodeCredentialClaimCreateResult::Created(_)
    ));

    let wrong_secret = crate::node_claim::NodeClaimSecret::from_test_bytes([0x55; 32]);
    assert_eq!(
        db.claim_node_credential(&test_claim_attempt(
            "claim-a",
            10,
    ...
    let claimed = db
        .claim_node_credential(&test_claim_attempt(
            "claim-a",
            10,
            "Node_A",
            secret.clone(),
        ...
        .await
        .unwrap();
    assert!(matches!(claimed, NodeCredentialClaimResult::Claimed(_)));
    ...
    let retry = db
        ...
        .await
        .unwrap();
    assert!(matches!(retry, NodeCredentialClaimResult::Existing(_)));
    ...
    assert_eq!(
        credential_count, 0,
        "Claim state must not mutate permanent credentials"
    );
}

#[tokio::test]
async fn node_credential_claim_expiry_cancel_and_recreation() {
    let db = repo().await;
    seed_group(&db, 10).await;
    let t0 = claim_test_time("2026-09-23T13:00:00Z");
    let node = crate::node_identity::ReuseEligibleNodeId::parse("Node_A").unwrap();

    let (old, old_secret) = test_node_claim("claim-old", 10, "Node_A", 0x31, 1, t0, 60);
    assert!(matches!(
    ...
    let (replacement, replacement_secret) = test_node_claim(
        "claim-new",
        10,
        "Node_A",
        0x32,
    ...
    assert!(matches!(
        db.create_node_credential_claim(&replacement).await.unwrap(),
        NodeCredentialClaimCreateResult::Created(_)
    ));
    assert_eq!(
        db.claim_node_credential(&test_claim_attempt(
...
#[tokio::test]
async fn node_credential_claim_concurrent_create_and_claim() {
    let db = repo().await;
    seed_user(&db, 2, true).await;
    seed_group(&db, 10).await;
    let now = claim_test_time("2026-09-23T14:00:00Z");
    let (a, secret_a) = test_node_claim("claim-race-a", 10, "Node_A", 0x51, 1, now, 600);
    let (b, secret_b) = test_node_claim("claim-race-b", 10, "Node_A", 0x52, 2, now, 600);
    ...
    let active: Vec<NodeCredentialClaimRecord> = db
        .list_node_credential_claims_for_identity(
            10,
            &crate::node_identity::ReuseEligibleNodeId::parse("Node_A").unwrap(),
        )
        .await
        .unwrap();
    assert_eq!(
    ...
    let winner = active
        ...
        .claim_id
        .clone();
    let winner_secret = if winner == "claim-race-a" {
        secret_a
    } else {
        secret_b
    };
    ...
    assert!(
        (matches!(ca, NodeCredentialClaimResult::Claimed(_))
            && cb == NodeCredentialClaimResult::Replay)
            || (matches!(cb, NodeCredentialClaimResult::Claimed(_))
                && ca == NodeCredentialClaimResult::Replay)
    );
    let rows = db
        .list_node_credential_claims_for_identity(
            10,
...
#[tokio::test]
async fn node_credential_claim_schema_constraints_and_fk_restrict() {
    let db = repo().await;
    sqlx::query("PRAGMA foreign_keys = ON")
        .execute(&db.pool)
        .await
        .unwrap();
    seed_group(&db, 10).await;
    let now = claim_test_time("2026-09-23T15:00:00Z");
    let (claim, _) = test_node_claim("claim-constraints", 10, "Case_Sensitive", 0x71, 1, now, 600);
    assert!(matches!(
        db.create_node_credential_claim(&claim).await.unwrap(),
        NodeCredentialClaimCreateResult::Created(_)
    ));
        ...
        .enumerate()
    {
        let result = sqlx::query(
            "INSERT INTO node_credential_claims \
             ...
             VALUES (?,10,?,'rp-node-claim-sha256',1,?,'APPROVED',?,1,'approval-x',?,?)",
        )
        .bind(format!("invalid-{index}"))
        .bind(invalid)
        .bind(vec![0x77_u8; 32])
    ...
    let duplicate = sqlx::query(
        "INSERT INTO node_credential_claims \
         (claim_id,home_group_id,node_id,secret_verifier_format,secret_verifier_version,secret_verifier_data,state,expires_at,approved_by,approval_ref,created_at,updated_at) \
         VALUES ('claim-duplicate',10,'Case_Sensitive','rp-node-claim-sha256',1,?,'APPROVED','2026-09-23T15:10:00Z',1,'approval-y','2026-09-23T15:00:00Z','2026-09-23T15:00:00Z')",
    )
    .bind(vec![0x78_u8; 32])
    .execute(&db.pool)
    .await;
    assert!(
        duplicate.is_err(),
        "partial unique index must block a second non-terminal Claim"
    );
    ...
    assert!(
        malformed.is_err(),
        "CLAIMED requires a pinned nonce verifier and claimed_at"
    );
    ...
    .await;
    assert!(
        malformed_terminal.is_err(),
        "claimed_at must never exist without a claimant nonce verifier"
    );
}

#[tokio::test]
async fn node_credential_claim_schema_upgrade_is_idempotent() {
    let db = repo().await;
    seed_group(&db, 30).await;
    sqlx::query(
        "INSERT INTO node_credentials \
         (credential_id, home_group_id, node_id, generation, verifier_format, verifier_version, verifier_data) \
         VALUES ('preserved-credential',30,'AAA',1,'rp-node-sha256',1,?)",
    )
    .bind(vec![0x91_u8; 32])
    .execute(&db.pool)
    .await
    ...
    let table_count: i64 = sqlx::query_scalar(
        "SELECT COUNT(*) FROM sqlite_master WHERE type='table' AND name='node_credential_claims'",
    )
    .fetch_one(&db.pool)
    .await
    .unwrap();
    let index_count: i64 = sqlx::query_scalar(
        "SELECT COUNT(*) FROM sqlite_master WHERE type='index' AND name='uq_node_credential_claims_one_nonterminal'",
    ...
    assert_eq!(
        credential_count, 1,
        "S2-A2A rows must survive Claim migration"
    );

    let too_long = test_node_claim(
        "claim-too-long",
        30,
        "AAA",
        0x92,
        1,
        claim_test_time("2026-09-23T16:00:00Z"),
...
async fn node_credential_claim_migration_failure_recovery() {
    ...
    for id in ["migration-race-a", "migration-race-b"] {
        sqlx::query(
            "INSERT INTO node_credential_claims \
             (claim_id,home_group_id,node_id,secret_verifier_format,secret_verifier_version,secret_verifier_data,state,expires_at,approved_by,approval_ref,created_at,updated_at) \
             VALUES (?,10,'Node_A','rp-node-claim-sha256',1,?,'APPROVED','2026-09-23T18:00:00Z',1,'approval-migration','2026-09-23T17:00:00Z','2026-09-23T17:00:00Z')",
        ).bind(id).bind(vec![0xA1_u8;32]).execute(&db.pool).await.unwrap();
    }
    assert!(crate::db::schema::run_migrations(&db.pool).await.is_err());
    let index_count: i64 = sqlx::query_scalar(
        "SELECT COUNT(*) FROM sqlite_master WHERE type='index' AND name='uq_node_credential_claims_one_nonterminal'",
    ).fetch_one(&db.pool).await.unwrap();
    assert_eq!(
        index_count, 0,
        "failed migration must not leave a partial unique index"
    );
...
fn new_dns_binding(rule_id: i64, record_id: &str) -> NewDnsRecordBinding {
    NewDnsRecordBinding {
        ...
        desired_value: "192.0.2.10".into(),
        state: "BOUND".into(),
        last_observed_at: Some("2026-08-26 00:00:00".into()),
        created_at: "2026-08-26 00:00:00".into(),
    }
}
