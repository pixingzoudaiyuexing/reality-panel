...
pub(crate) async fn discover_records(
    ...
    if !matches!(filtered_result, RecordDiscovery::NoRecord) {
        return if filtered.complete {
            ...
            RecordDiscovery::UpstreamFailure(DnsMgrError::ProtocolContractViolation(
                "DNSMgr returned an incomplete filtered record inventory".into(),
            ))
        };
    }
...
async fn fetch_record_inventory(
    ...
    loop {
        ...
        if reached_total || inferred_complete {
            complete = true;
            break;
        }
        if count == 0 {
    ...
    Ok(RecordInventory { records, complete })
}
...
mod tests {
    ...
    #[tokio::test]
    async fn record_discovery_filtered_empty_and_complete_full_empty_is_no_record() {
        let router = Router::new().route(
            "/api/record/data/7",
            post(|Form(_form): Form<HashMap<String, String>>| async move {
    ...
    async fn carrier_preflight_recovery_rejects_multiple_claimed_and_bound_replacements() {
        ...
        configure_eligible_rule(&multiple_db, "op1.example.com", "192.0.2.10").await;
        insert_line_binding(&multiple_db, "Dianxin", "stale-id", "192.0.2.20").await;
        let multiple_binding = multiple_db
            .find_dns_record_binding_for_rule(100, "op1.example.com", "A", "dnsmgr:Dianxin")
            .await
            .unwrap()
            .unwrap();
        ...
        let multiple = spawn_ensure_mock(
            vec![
                record("replacement-1", "A", "192.0.2.20", "Dianxin"),
                record("replacement-2", "A", "192.0.2.20", "Dianxin"),
            ],
            MutationBehavior::Apply,
            MutationBehavior::Apply,
        )
        .await;
        assert!(matches!(
            inspect_line_record(&multiple_db, &multiple.client, 100, "Dianxin").await,
            Err(LineRecordSnapshotError::OwnershipUnverified)
        ));
        let still_multiple = multiple_db
            .find_dns_record_binding_for_rule(100, "op1.example.com", "A", "dnsmgr:Dianxin")
            .await
            .unwrap()
            .unwrap();
        assert_eq!(still_multiple.record_id, "stale-id");
        assert_eq!(still_multiple.state, "ERROR");
        assert_eq!(multiple.state.total_mutations(), 0);

        let claimed_db = ensure_db().await;
        configure_eligible_rule(&claimed_db, "op1.example.com", "192.0.2.10").await;
        insert_line_binding(&claimed_db, "Dianxin", "stale-id", "192.0.2.20").await;
        let claimed_stale = claimed_db
            .find_dns_record_binding_for_rule(100, "op1.example.com", "A", "dnsmgr:Dianxin")
            .await
            .unwrap()
            .unwrap();
        ...
        claimed_db
            .insert_dns_record_binding(&NewDnsRecordBinding {
                ...
                record_id: "replacement-id".into(),
                desired_value: "192.0.2.20".into(),
                state: "BOUND".into(),
                last_observed_at: Some("2026-09-25 00:00:00".into()),
                created_at: "2026-09-25 00:00:00".into(),
            })
            .await
            .unwrap();
        let claimed = spawn_ensure_mock(
            vec![record("replacement-id", "A", "192.0.2.20", "Dianxin")],
            MutationBehavior::Apply,
            MutationBehavior::Apply,
        )
        .await;
        assert!(matches!(
            inspect_line_record(&claimed_db, &claimed.client, 100, "Dianxin").await,
            Err(LineRecordSnapshotError::OwnershipUnverified)
        ));
        let still_claimed = claimed_db
            .find_dns_record_binding_for_rule(100, "op1.example.com", "A", "dnsmgr:Dianxin")
            .await
            .unwrap()
            .unwrap();
        assert_eq!(still_claimed.record_id, "stale-id");
        assert_eq!(still_claimed.state, "ERROR");
        assert_eq!(claimed.state.total_mutations(), 0);

        let bound_db = ensure_db().await;
        configure_eligible_rule(&bound_db, "op1.example.com", "192.0.2.10").await;
        insert_line_binding(&bound_db, "Dianxin", "stale-id", "192.0.2.20").await;
        let bound = spawn_ensure_mock(
            vec![record("replacement-id", "A", "192.0.2.20", "Dianxin")],
            MutationBehavior::Apply,
            MutationBehavior::Apply,
        )
        .await;
        assert!(matches!(
            inspect_line_record(&bound_db, &bound.client, 100, "Dianxin").await,
            Err(LineRecordSnapshotError::OwnershipUnverified)
        ));
        let still_bound = bound_db
            .find_dns_record_binding_for_rule(100, "op1.example.com", "A", "dnsmgr:Dianxin")
            .await
            .unwrap()
            .unwrap();
        assert_eq!(still_bound.record_id, "stale-id");
        assert_eq!(still_bound.state, "BOUND");
        assert_eq!(still_bound.last_error_category, None);
        assert_eq!(bound.state.total_mutations(), 0);
    }

    #[tokio::test]
    async fn carrier_preflight_does_not_rebind_when_provider_line_changed() {
        let db = ensure_db().await;
        configure_eligible_rule(&db, "op1.example.com", "192.0.2.10").await;
        insert_line_binding(&db, "Dianxin", "stale-id", "192.0.2.20").await;
        let stale = db
        ...
        let changed_line = spawn_ensure_mock(
            vec![record(
                "replacement-id",
                "A",
                "192.0.2.20",
        ...
        .await;
        assert_eq!(
            inspect_line_record(&db, &changed_line.client, 100, "Dianxin")
                .await
                .unwrap(),
            LineRecordSnapshot::Absent
        );
        ...
        assert_eq!(changed_line.state.total_mutations(), 0);
    }
    ...
    #[tokio::test]
    async fn carrier_apply_starts_with_one_default_and_operator_line_records() {
        ...
        assert_eq!(
            stored
                .dns_records
                .iter()
                .filter(|record| record.line_key == DEFAULT_LINE_KEY)
                .count(),
            1
        );
        assert_eq!(mock.state.total_mutations(), 0);
    ...
    async fn normal_carrier_roundtrip_commits_one_journal_and_retains_configuration() {
        ...
        db.set("node_status:10:node-a", &status("192.0.2.20"))
            .await
            .unwrap();
        db.set("node_status:10:node-b", &status("192.0.2.30"))
            .await
            .unwrap();
        let policy = CarrierPolicy {
        ...
        assert_eq!(pending.pending_routing_mode, Some(RoutingMode::Carrier));
        assert_eq!(pending.dns_records.len(), 4);
        assert_eq!(
            pending
                .dns_records
                .iter()
        ...
        crate::service::relay_preference::finalize_switching_group_for_test(&db, &connections, 10)
            .await
            .unwrap();
        let carrier: RelayPreferenceState =
            serde_json::from_str(&db.get("relay_preference:10").await.unwrap().unwrap()).unwrap();
        assert_eq!(carrier.active_routing_mode, Some(RoutingMode::Carrier));
        ...
        assert_eq!(returning.pending_routing_mode, Some(RoutingMode::Normal));
        for sync in db.list_dns_record_syncs_for_rule(100).await.unwrap() {
            reconcile_one(&db, sync, &mock.client).await;
        }
        crate::service::relay_preference::finalize_switching_group_for_test(&db, &connections, 10)
            .await
            .unwrap();
        let normal: RelayPreferenceState =
            serde_json::from_str(&db.get("relay_preference:10").await.unwrap().unwrap()).unwrap();
