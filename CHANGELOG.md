...
- 修复线路配置已由后端成功接收，但请求超时或状态刷新失败时前端误报“线路配置处理失败”的问题。
- 为线路配置提交使用独立超时，并在结果未知时进行有限次数的只读状态刷新，不会自动重复提交。

### 兼容性

- 无数据库 migration，无 backend、DNS 或 Node 功能变化。
- Config Protocol 保持 `10`，Lifecycle Protocol 保持 `1`。
- v1.1.16 可直接升级到 v1.1.17。
...
- 将常规默认、运营商规则、定时规则和故障切换规则统一为只切换配置视图的线路功能 Tab，不再因浏览配置而立即切换运行模式。
- 新增统一的保存/启用协调接口，继续复用现有线路事务、Provider 回读、回滚和最终确认机制。
- 优化当前生效模式、切换状态、未保存草稿和业务错误提示；配置保存成功但启用失败时保留配置并明确显示部分失败。

### 兼容性

- 无数据库 migration。
- Config Protocol 保持 `10`，Lifecycle Protocol 保持 `1`。
- v1.1.14 可直接升级到 v1.1.15；Relay Node 无功能代码变化。

## [1.1.14] - 2026-09-09

### 新增

- Group 线路统一为常规、运营商、定时切换和故障转移四种运行模式，并确保同一时间只有当前模式拥有默认线路控制权。
- 运营商模式支持独立设置全网默认节点，并将全网默认与运营商专线作为同一安全 DNS 事务提交。
- 节点状态页默认收起 Group，支持单组展开并记住上次展开状态。

### 修复

- 阻止已退出模式的后台任务继续修改默认线路，并在 Provider 回读成功后才提交模式切换。
- 完整清理无 DNS 模式切换后的旧事务状态，避免已回滚 journal 污染后续线路操作。

### 兼容性

- 无数据库 migration。
- Config Protocol 保持 `10`，Lifecycle Protocol 保持 `1`。
- v1.1.13 可直接升级到 v1.1.14；Relay Node 无功能代码变化。
...
- 节点生命周期操作改为后台任务视图，关闭详情窗口或离开页面不会中断任务。
- 支持后端持久化的全节点滚动升级，每次只升级一个节点，并记录成功、失败和跳过结果。

### 修复

- 根据 Restart、Upgrade 和 Uninstall 动作显示准确的验证阶段文案。
- Batch 状态转换写入失败时使用有上限间隔持续重试，避免任务静默退出、重复升级或永久阻塞。

### 兼容性

- 无数据库 migration。
- Config Protocol 保持 `10`，Lifecycle Protocol 保持 `1`。
- v1.1.11 可直接升级到 v1.1.12。
