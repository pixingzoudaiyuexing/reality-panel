...
export function CarrierAffinityPanel({ groupId, nodes, t, onViewChange, onCatalogChange, onAvailabilityChange }: Props) {
  ...
  const defaultNodeId = nodes.find((node) => node.preferred)?.node_id ?? view?.default_node_id;
  ...
  const assignLines = (nodeId: string, selected: string[]) => {
    setDraft((current) => assignCarrierLines(current, nodeId, selected, defaultNodeId));
  };
  ...
  const save = async () => {
    ...
    setSaving(true);
    try {
      const response = await api.put<unknown, ApiEnvelope<CarrierAffinityView>>(`/groups/${groupId}/carrier-affinity`, { bindings: mutableCarrierBindings(draft) });
          ...
          {nodes.map((node) => (
            <div className="rp-carrier-node-grid-row" role="row" data-testid={`carrier-node-${node.node_id}`} key={node.node_id}>
              ...
              <Space orientation="vertical" size={4} style={{ width: '100%' }}>
                {defaultNodeId === node.node_id ? <Tag color="blue" data-testid="carrier-default-node-indicator">{t('carrierAllNetworkDefault')}</Tag> : null}
                <Select mode="multiple" showSearch aria-label={`${node.node_id} ${t('carrierLine')}`} value={nodeLines(draft, node.node_id, defaultNodeId)} disabled={mutationLocked || catalogUnavailable} placeholder={t('carrierNotConfigured')} options={lineOptions} filterOption={(query, option) => carrierLineMatchesSearch(query, { value: String(option?.value ?? ''), label: String(option?.label ?? '') })} onChange={(values) => assignLines(node.node_id, values)} style={{ width: '100%' }} />
              </Space>
            </div>
          ))}
        </div>
